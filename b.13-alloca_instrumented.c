...
int test_fun(int x, int y, int z)
{
 ...
 while ((*x_ref > *z_ref) || (*y_ref > *z_ref)) {printf("CBMC Instrumentation @ line557");static myBool pStored = myFalse;myBool flag=__VERIFIER_nondet_myBool();static int * oc;static int ox;static int * ox_ref;static int oy;static int * oy_ref;static int oz;static int * oz_ref;if(pStored){__CPROVER_assert(!(oc==c && *oc==*c && ox==x && ox_ref==x_ref && *ox_ref==*x_ref && oy==y && oy_ref==y_ref && *oy_ref==*y_ref && oz==z && oz_ref==z_ref && *oz_ref==*z_ref),"recurrent state found");} if(flag){oc=c;*oc=*c;ox=x;ox_ref=x_ref;*ox_ref=*x_ref;oy=y;oy_ref=y_ref;*oy_ref=*y_ref;oz=z;oz_ref=z_ref;*oz_ref=*z_ref;pStored=myTrue;}  __VERIFIER_loop_head(557, " ");

        __VERIFIER_reached_control(558, " ");
 if (*x_ref > *z_ref) { __VERIFIER_control_true(558, " ");

            *x_ref = *x_ref - 1;
        } else { __VERIFIER_control_false(560, " ");

            __VERIFIER_reached_control(561, " ");
 if (*y_ref > *z_ref) { __VERIFIER_control_true(561, " ");

                *y_ref = *y_ref - 1;
            } else { __VERIFIER_control_false(563, " ");
