...
int avg (int x[N])
{
  ...
  long long ret;
  ret = 0;
  for (i = 0; i < N; i++) {printf("ArrayNT State @ line26: Array Found\n");
    ret = ret + x[i];
  }
  return ret / N;
}
...
int main ()
{
  ...
  for (int i = 0; i < N; i++) {printf("ArrayNT State @ line40: Array Found\n");
  ...
  ret2 = fun(x);
  temp=x[0];
  for(int i =0 ; i<N-1; i++){printf("ArrayNT State @ line49: Array Found\n");
     x[i] = x[i+1];
  }
  x[N-1] = temp;
  ret5 = fun(x);
